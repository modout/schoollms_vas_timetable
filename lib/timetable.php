...
function get_url($SERVER){
    ...
    $myUrl = (isset($SERVER['HTTPS']) && $SERVER['HTTPS'] && !in_array(strtolower($SERVER['HTTPS']),array('off','no'))) ? 'https' : 'http';
    // Get domain portion
    $myUrl .= '://'.$SERVER['HTTP_HOST'];
    // Get path to script
    $myUrl .= $SERVER['REQUEST_URI'];
    // Add path info, if any
    if (!empty($SERVER['PATH_INFO'])) $myUrl .= $SERVER['PATH_INFO'];
    // Add query string, if any (some servers include a ?, some don't)
    if (!empty($SERVER['QUERY_STRING'])) $myUrl .= '?'.ltrim($SERVER['REQUEST_URI'],'?');

    return $myUrl;
}

function navigation_menu ($school_id, $startdate=null,$today=null, $user_id=null, $data=null){
...
//    $menu .= "<td><a href='$protocol://$ip/vas/timetable/?startdate=1&today=$today'> <img src='./images/arrow_right_22x22.png'> </a></td></tr></table>";
    ...
    $menu = "<table><tr><td><a href='$protocol://$ip$url&startdate=-1&today=$today'><img src='./images/arrow_left_22x22.png'></a></td>";                                        
    $menu .= "<td><button style='background-color:transparent; border-color:transparent;' id='btnMenu' name='btnMenu'  onClick='openMenu()'><img src='./images/menu2_26x22.png'></button></td>";
    $menu .= "<td><a href='$protocol://$ip$url&startdate=1&today=$today'> <img src='./images/arrow_right_22x22.png'> </a></td></tr></table>";

    return $menu;
}

//Prepare School Time Table View for user;
function prepareTableView($user_id, $time_table_id,$user_type, $year_id, $school_id, $parent_id){
    //echo "Time table ID : $time_table_id <br/>";
    global $timetable_id;
    ...
    switch ($user_type){
        ...
        case 6:
            $q = "SELECT timetabl_id FROM schoollms_schema_userdata_school_timetable_items WHERE timetabl_id = $time_table_id AND year_id = $year_id";
            break;
    }
        ...
        foreach ($timetable_result as $items) {
            ...
            switch($user_type){
                ...
                case 2://CHECK IF LEARNER TIMETABLE EMPTY
                    $q = "SELECT * FROM schoollms_schema_userdata_school_timetable_items WHERE timetabl_id = $timetable_id AND year_id = $year_id";
                ...
                case 4://CHECK IF TEACHER TIMETABLE EMPTY
                    ...
                    if (count(sqlQuery($q)) == 0){
                        //echo "BUILD $timetable_id <br>";
                        $timetable_id = build_teacher_timetable($user_id, $year_id, $timetable_id);
                    } else {
                        //echo "UPDATE TIMETABLE $timetable_id <br>";
                        $timetable_id = update_teacher_timetable($user_id, $year_id, $timetable_id);
                    }
                    break;
            }
...
//      echo "TEST REGISTRATION ITEMS ".$s_items[0][1]."<br>";      
      $settings_tokens = explode("|", $s_items[0][1]);
      $register_tokens = explode ("=", $settings_tokens[11]);
//      echo "<b> REGISTER TOKEN ".$settings_tokens[11]."</b></br>";
      if ($register_tokens[1] == 'Yes'){
//          echo "ECHO HAS REGISTRATION <br>";
          $has_registration = TRUE;
      }
      break;
    }
   // echo "HREGISTRATION <br>";
//    exit;
    ...
    room_id, subject_id, teacher_id, substitude_id from schoollms_schema_userdata_school_timetable_items where timetabl_id = $timetable_id and year_id= $year_id and day_id =  $timetable_day and period_label_id = $period_label_id";
                  //  break;
...
function user_stats($timetable_id, $timetable_day, $date_stamp, $period_label_id, $grade_id, $class_id, $room_id, $subject_id, $teacher_id, $substitute_id, $user_type, $user_id){
    ...
    switch ($user_type) {
        case 2:
            $stats = get_learner_stats($timetable_id, $timetable_day, $date_stamp, $period_label_id, $grade_id, $class_id, $room_id, $subject_id, $teacher_id, $substitute_id, $user_type, $user_id);
            break;

        default:
            break;
    }
    return @"<b>$subject</b><br>$teacher<br>$room <br>~ $stats";
    
}

function get_learner_stats($timetable_id, $timetable_day, $date_stamp, $period_label_id, $grade_id, $class_id, $room_id, $subject_id, $teacher_id, $substitute_id, $user_type, $user_id){

    
    $stats = "<b> PERIOD STATS </b><br> Attendance: PRESENT <br> Tablet: PRESENT <br> Merits: 0 <br> Demerits: 0 <br> Lesson Access: 0 <br> Study Note: 0";
...
function update_teacher_timetable($user_id, $year_id, $timetable_id){
    
        $q = "SELECT * FROM schoollms_schema_userdata_school_timetable_items WHERE teacher_id = $user_id";
    $result = sqlQuery($q);
    ...
    if ($update_found){
        $q = "SELECT * FROM schoollms_schema_userdata_teacher_schooldetails WHERE user_id = $user_id AND year_id = $year_id";

        $result = sqlQuery($q);

        $periods = array ();
        $days = array ();
        $day_period_found = array ();
        ...
        foreach ($result as $items) {
            foreach ($items as $item) {
                ...
                if ($class_id == 0){
                    ...
                    $q = "SELECT * FROM schoollms_schema_userdata_school_subjects WHERE subject_id = $subject_id";
                    //echo "SUBJECTS Q $q <br>";
                    $subject_result = sqlQuery($q);
                    foreach($subject_result as $subject_items){
                        $subject_title = $subject_items[0][1];
                        break;
                    }

                    $q = "SELECT * FROM schoollms_schema_userdata_school_classes WHERE school_id = $school_id AND year_id = $year_id AND grade_id = $grade_id AND class_label LIKE '%$subject_title%'";
                    ...
                    if (count($subject_result) > 0){
                        $subject_class = TRUE;
                        //echo "FOUND SUBJECT CLASS NAME <br>";
                        foreach($subject_result as $subject_items){
                            foreach($subject_items as $subject_item){
                                $class_id = $subject_item[3];
                                $classes [] = $class_id;
                                //break;
                            }
                        }
                ...
                foreach ($classes as $key=>$class_id){
                    ...
                    if (count($result2) > 0){
                        ...
                        $q = "SELECT * FROM schoollms_schema_userdata_school_timetable_items WHERE timetabl_id = $temp_timetable_id ORDER BY day_id, period_label_id";

                        //echo "SELECT CLASS TIMETABLE ITEMS $temp_timetable_id SQL $q <br>";
                        ...
                        foreach ($result2 as $items2) {
                            foreach ($items2 as $row){
                                ...
                                if ($subject_class){
                                    $q = "SELECT * FROM schoollms_schema_userdata_teacher_schooldetails WHERE user_id = $user_id AND year_id = $year_id AND grade_id = $grade_id AND class_id = $class_id";
                                } else {
                                    $q = "SELECT * FROM schoollms_schema_userdata_teacher_schooldetails WHERE user_id = $user_id AND year_id = $year_id AND grade_id = $grade_id AND subject_id = $subject_id";
                                }
                                //echo "TEACHER SCHOOLDETAILS ID Q $q <br>";
                                $result3 = sqlQuery($q);

                                if (count($result3) > 0){

                                    foreach ($result3 as $result3_items) {
                                        $result_class_id = $result3_items[0][3];
                                        $result_subject_id = $result3_items[0][4];
                                        if ($result_class_id == 0){
                                            $q = "UPDATE schoollms_schema_userdata_teacher_schooldetails SET class_id = $class_id WHERE user_id = $user_id AND year_id = $year_id AND grade_id = $grade_id AND subject_id = $subject_id";
                                            $temp_result = sqlQuery($q);
                                        }
                                        break;
                                    }

                                    $q = "SELECT * FROM schoollms_schema_userdata_school_timetable_items WHERE timetabl_id = $timetable_id AND day_id = $day_id AND period_label_id = $period_label_id AND grade_id = $grade_id AND class_id = $class_id AND subject_id = $subject_id AND teacher_id = $user_id AND last_changed = '$last_changed'";

                                    //echo "TEACHER TIMETABLE ITEMS Q $q <br>";

                                    $result4 = sqlQuery($q);

                                    //echo "COUNT ".count($result4)."<br>";

                                    if (count($result4) > 0){
                                        //$q = "NO QUERY";
                                        //$q = "UPDATE schoollms_schema_userdata_school_timetable_items SET grade_id = $grade_id, class_id = $class_id, room_id = $room_id, subject_id = $subject_id , teacher_id = $user_id, substitude_id = $substitude_id, last_changed = '$last_update' WHERE timetabl_id = $timetable_id AND day_id = $day_id AND period_label_id = $period_label_id";
                                    } elseif (count($result4) == 0 && $teacher_id == $user_id) {
                                        $q = "DELETE FROM schoollms_schema_userdata_school_timetable_items WHERE timetabl_id = $timetable_id AND day_id = '$day_id' AND period_label_id = '$period_label_id'";
                                        $temp_result = sqlQuery($q);
                                        $q = "INSERT INTO schoollms_schema_userdata_school_timetable_items VALUES ($timetable_id, $day_id, $period_label_id, $grade_id, $class_id, $room_id, $subject_id, $user_id, $substitude_id, '$last_changed')";
                                    } elseif (count($result4) == 1) {
                                        $q = "UPDATE schoollms_schema_userdata_school_timetable_items SET grade_id = $grade_id, class_id = $class_id, room_id = $room_id, subject_id = $subject_id , teacher_id = $user_id, substitude_id = $substitude_id, last_changed = '$last_changed' WHERE timetabl_id = $timetable_id AND day_id = $day_id AND period_label_id = $period_label_id";
                                    }
                                    //echo "FINAL Q $q <br>";
                                    $temp_result = sqlQuery($q);

                                    //TRACK ALL SEEN DAYS AND CORRESPONDING PERIODS
                                    //$days["$day_id - $period_label_id"] = $day_id; 
                                    //$periods["$day_id - $period_label_id"] = $period_label_id;
                                    $day_period_found[] = "$day_id-$period_label_id";
                                    //echo "<br> FOUND D_P $day_id-$period_label_id <br>";
                                } else {
                                    $q = "SELECT * FROM schoollms_schema_userdata_school_timetable_items WHERE timetabl_id = $timetable_id AND day_id = $day_id AND period_label_id = $period_label_id AND subject_id IN (SELECT subject_id FROM schoollms_schema_userdata_teacher_schooldetails WHERE user_id = $user_id AND year_id = $year_id AND grade_id = $grade_id)";
                                    $temp_result = sqlQuery($q);
                                    $result_count = count($temp_result);
                                    //echo "COUNT $result_count Q $q <br>";
                                    if ($result_count > 0){
                                        $day_period_found[] = "$day_id-$period_label_id";
                                        //echo "<br> FOUND D_P $day_id-$period_label_id <br>";
                                    }
        ...
        foreach ($result as $result_items) {
            foreach ($result_items as $items) {
                $day_id = $items[1];
                $period_label_id = $items[2];
                $q = "SELECT * FROM schoollms_schema_userdata_school_timetable_period_labels WHERE school_id = $school_id AND period_label_id = $period_label_id AND period_label = 'Registration'";
                if (!in_array("$day_id-$period_label_id", $day_period_found) && count(sqlQuery($q)) == 0){
                    //$q = "DELETE FROM schoollms_schema_userdata_school_timetable_items WHERE timetabl_id = $timetable_id AND day_id = '$day_id' AND period_label_id = '$period_label_id'";
                    //echo "DELETE Q $day_id-$period_label_id <br>";
                    $temp_result = sqlQuery($q);
                }
    ...
    foreach ($result as $items) {
        foreach ($items as $item){
            ...
            if (count($result2) > 0){
                ...
                $q = "SELECT * FROM schoollms_schema_userdata_school_timetable_items WHERE timetabl_id = $temp_timetable_id AND year_id = $year_id";
                ...
                foreach ($result2 as $items2) {
                    foreach ($items2 as $row){
                        ...
                        $q = "INSERT INTO schoollms_schema_userdata_school_timetable_items VALUES ($timetable_id, $year_id,$day_id, $period_label_id, $grade_id, $class_id, $room_id, $subject_id, $teacher_id, 0, '$last_update')";
                ...
                $q = "SELECT * FROM schoollms_schema_userdata_school_classes WHERE class_id = $class_id";
                
                $result2 = sqlQuery($q);
                foreach ($result2 as $items2) {
                    $class_label = $items2[0][4];
                    break;
                }
                
                $class_tokens = explode(" ", $class_label);
                
                if (strtoupper($class_tokens[0]) == "CLASS"){
                    $q = "INSERT INTO schoollms_schema_userdata_school_timetable VALUES (NULL, $school_id, 3, $class_id, '$class_label', NULL)";
                } else {
                    $q = "INSERT INTO schoollms_schema_userdata_school_timetable VALUES (NULL, $school_id, 8, $class_id, '$class_label', NULL)";
                }
                sqlQuery($q);
                
                //$timetable_id = build_learner_timetable($user_id, $year_id, $timetable_id);
                //echo "Q $q <br> CLASS $class_label ID $class_id <br>";
                //exit;
            }
        }
    }
...
function build_teacher_timetable($user_id, $year_id){
    ...
    //CREATE TEACHER TIMETABLE ID
    $q = "SELECT * FROM schoollms_schema_userdata_school_timetable WHERE timetable_type_id = 2 AND timetable_type_item_id = $user_id";
    $result = sqlQuery($q);
    ...
    $q = "select * from schoollms_schema_userdata_school_timetable_items where teacher_id = $user_id AND year_id = $year_id order by day_id, period_label_id asc";
    ...
    foreach ($timetable_items as $items) {
        ...
        foreach ($items as  $item){
            ...
            $year_id = $item[1];
            $day_id = $item[2];
            $period_label_id = $item[3];
            $grade_id = $item[4];
            $class_id = $item[5];
            $room_id = $item[6];
            $subject_id = $item[7];
            $teacher_id = $item[8];
            $substitute_id = $item[9];
            ...
            $q = "SELECT * FROM schoollms_schema_userdata_school_timetable_items WHERE timetabl_id = $timetable_id AND year_id = $year_id AND day_id = $day_id AND period_label_id = $period_label_id AND grade_id =  $grade_id AND class_id = $class_id AND room_id = $room_id AND subject_id = $subject_id AND teacher_id = $user_id";
            
            $result = sqlQuery($q);
            
            //echo "CHECK TIMETABLE Q $q<br>";
            
            if (count($result) == 0 && $teacher_id == $user_id){
                $q = "insert into schoollms_schema_userdata_school_timetable_items (timetabl_id, year_id, day_id, period_label_id, grade_id, class_id, room_id, subject_id, teacher_id, substitude_id) values ($timetable_id, $year_id, $day_id, $period_label_id, $grade_id, $class_id, $room_id, $subject_id, $user_id, $substitute_id)";
...
function print_days($time_table_id = 22, $user_type, $user_id, $year_id, $school_id, $startdate=null, $today=null, $data=null, $parent_id=null){
...
//    exit;
    ...
    foreach ($days as $key=>$day) {
        ...
        //GET DAY
        print " <tr height='142px'><td class='mark dark'><div class='tableFirstCol'> $day_string </div></td></tr>";
        //GET DAY SLOTS
        $slots[$key] = print_day($day_string, $key+1,$time_table_id, $user_type, $user_id, $year_id, $school_id, $data, $parent_id);
    }
...
//        //var_dump($days);
//        //REMOVE PREVIOUS TIMETABLE LOAD IF FIRST DATE IS DIFFERENT TO TODAY
//        $timetable_day_items = sqlQuery("select * from schoollms_schema_userdata_school_timetable_days where school_id = $school_id");
//
//        $day_label = "";
//        foreach ($timetable_day_items as $day_items){
//            $day_tokens = explode(" ", $day_items[0][2]);
//            $day_label = $day_tokens[0]; 
//            break;
//        }
//
//        //FOR EACH DAY LOAD USER TIMETABLE
...
//            //GET DAY LABEL
//            //$timetable_days = sqlQuery("select * from schoollms_schema_userdata_school_timetable_days where school_id = $school_id");
//            $day_string = "<b>$day_label $timetable_day</b><br>$day_items[0]<br>$day_items[1]-$month_items[0]-$year";
//            //GET DAY ITEMS
//
...
function periods ($school_id, $startdate=null,$today=null){
    ...
    $sql = "select l.*, p.period_start, p.period_end 
		from schoollms_schema_userdata_school_timetable_period_labels l
		join schoollms_schema_userdata_school_timetable_period p on p.period_label_id = l.period_label_id and l.school_id = p.school_id and l.year_id = p.year_id
		where l.school_id = $school_id AND l.year_id =  4
		and week_day_id = 1
		order by UNIX_TIMESTAMP(concat('2017-01-01',' ',period_start))  asc";
    //$period_labels = sqlQuery("select * from schoollms_schema_userdata_school_timetable_period_labels where school_id = $school_id order by period_label_id ASC");
	$period_labels = sqlQuery($sql);
...
//            break;
//        }
        $start = $period_label[0][5];
        $end = $period_label[0][6];
        if ($label == "Registration" && !$has_registration){
            continue;
        }
        
        print "<td class=\"redips-mark dark\"><div class='tableHeader'><b>$label </b> $start-$end</div></td>";
...
function days($today, $school_id=null){
   ...
   // echo "NUM TABLE DAYS $num_timetable_days";
    ...
    $today_day = getTodayTimeTableDay($num_days, $num_timetable_days, $school_id);
...
//    if (!empty($year_start_date) || count($year_start_date) > 0){
//        foreach ($year_start_date as $start_date) {
//            $start = $start_date[0][0];
//        }
//
...
function getTodayTimeTableDay($num_days, $num_timetable_days, $school_id){
//    global $data;
//    
//    $q = "SELECT settings FROM schoollms_schema_userdata_school_week_days_settings WHERE school_id = $school_id";
//    $data->execSQL($q);
//    $row = $data->getRow();
//    $settings = $row->settings;
//    $settings_tokens = explode("=", $settings);
//    $start_day = $settings_tokens[1];
...
//        $q = "SELECT lesson_url,lesson_title FROM schoollms_schema_userdata_school_timetable_subject_lessons WHERE day_id = $day_id AND period_label_id = $period_label_id AND lesson_date = '$today_token1'";
        ...
        if (count($result) > 0){
			...
			$ip = $_SERVER["SERVER_ADDR"] == '154.0.172.207' ? 'www.schoollms.net': $_SERVER["SERVER_ADDR"];
			$port = $_SERVER['SERVER_PORT'];
			if($port != 80)
			{
				$ip = "$ip:$port";
                ...
                case 2:
                   ...
                   // }
					...
					foreach ($lessons as $lesson_url => $lesson_title) {
                        ...
                        $pars = "action=open_link&lms_db=$database&q=$lesson_url&username=$username&passwd=$passwd&date=$date_stamp&day=$day_id&period_label_id=$period_label_id&user_type=$user_type&user_id=$user_id&lesson_title=$lesson_title[0]";
...
//                    $q = "SELECT lesson_url,lesson_title,subject_id FROM schoollms_schema_userdata_school_timetable_training_lessons WHERE type_id = $user_type AND day_id = $day_id AND period_label_id = $period_label_id AND class_id = $class_id AND date_format(str_to_date(lesson_date,'%d-%b-%Y'),'%d-%m-%Y') = '$today_token2'";
//
//                    $result = sqlQuery($q);
            ...
            //CHECK FOR TRAINING LINKS
            
            $links .= "<b><i> RESOURCE LIBRARY:</i> </b> <br> <a href='http://172.16.0.3/#content_Resource~~~Library' target='_blank'> TYB </a> <br> <a href='http://seva.co.za' target='_blank'> SEVA </a><br><br>";
            return $links;
        } else {
            return $links;
        }
...
function print_day($day, $row,$time_table_id,$user_type, $user_id, $year_id, $school_id, $data, $parent_id) {
	global $rs;
        //global $user_id;
        global $timetable_id;
        
        //$day_slots = array ();
        //echo "print_day($time_table_id,$user_type, $user_id, $year_id, $school_id)<br/>";
	// if $rs is null than query database (this should be only first time)
	//echo "before RS setup<br>";
        //var_dump($rs);
        //exit;
	if ($rs == null) {
		//echo "<br/>Then it is here<br/>";
                switch($user_type){
                    case 2:
                    case 4:
                        $time_table_id = prepareTableView($user_id,$time_table_id,$user_type, $year_id, $school_id, $parent_id);
                        break;
...
//        exit;
        ...
        {
            //echo "LOAD PERiOds <br>";
            
                $period_labels[$index] = $row1->period_label_id;
                $period_label_titles [$index] = $row1->period_label;
                $index++;
        }
	...
	//echo "COUNT PERIOD LABELS ".count($period_labels);
	//echo "<br/>";
        $return_slots = "";
        if ($row == 1){
            $return_slots .= "<tr id='firstTr' height='142px'>";
        } else {
            $return_slots .= "<tr height='142px'>";
        }
//	print '<tr>';
...
//                }
		...
		// create table cell
		$return_slots .= '<td style="vertical-align:top"><div style="width:100%; max-height:142px; overflow:auto">';
...
//	// column loop starts from 1 because column 0 is for hours
//	$result = "";
//	for ($col=0; $col < count($period_labels); $col++) {
//		// create table cell
//                if ($period_label_titles[$col] == 'Registration' && !$has_registration){
//                    continue;
//                }
//                
//		print '<td>';
//		// prepare position key in the same way as the array key looks
//		$pos = $row . '_' . $period_labels[$col];
//		// if content for the current position exists
//		//echo "pos $pos <br/>";
//		//var_dump($rs);
//		//$keys = array_keys($rs);
...
//				if($user_type == 6)
//				{
//					print "<div id=\"$id\" class=\"drag $class\" onClick=\"viewTimeTableSlot('$dname[0]','$datas[0]','CLASS $dclass','$start-$end~$day~$user_id~$theperiod~$time_table_id')\" >$dname[0]</div>
//					$removeSlot
//					<div class=\"drag $class\">".urldecode($dname[1])."</div>";
//                                } elseif ($subject_class){
//                                        print "<div id=\"$id\" name=\"$id\" class=\"thecontextmenu drag $class\" onClick=\"newViewTimeTableSlot('$dname[0]::$datas[0]::CLASS $dclass::$start-$end~$day~$user_id~$theperiod~$time_table_id::$subject_class_name',this)\" >$dname[0]</div>
//					$removeSlot
//					<div class=\"drag $class\">".urldecode($dname[1])."</div>";
//                                } else {
//					print "<div id=\"$id\" name=\"$id\" class=\"thecontextmenu drag $class\" onClick=\"newViewTimeTableSlot('$dname[0]::$datas[0]::CLASS $dclass::$start-$end~$day~$user_id~$theperiod~$time_table_id',this)\" >$dname[0]</div>
//					$removeSlot
//					<div class=\"drag $class\">".urldecode($dname[1])."</div>";
//				}
//			}
                ...
                if (!empty($user_result)){
                    ...
                    switch ($user_type) {
                        case 2:
                            ...
                            $sql = "SELECT * FROM schoollms_schema_userdata_school_timetable_items WHERE timetabl_id = $learner_timetable_id AND year_id = $year_id AND day_id = $day_id AND period_label_id = $period_label_id";
                            $timetable_settings = sqlQuery($sql);

                            if (!empty($timetable_settings)) {
                                $sql = "UPDATE schoollms_schema_userdata_school_timetable_items SET grade_id = $grade_id, class_id = $class_id ,room_id = $room_id , subject_id = $subject_id,teacher_id = $teacher_id , substitude_id = 0 WHERE timetabl_id = $learner_timetable_id AND year_id = $year_id AND day_id = $day_id AND period_label_id = $period_label_id";
                            } else {
                                $sql = "INSERT INTO schoollms_schema_userdata_school_timetable_items (timetabl_id,year_id,day_id, period_label_id, grade_id, class_id,room_id, subject_id,teacher_id,substitude_id) VALUES ($learner_timetable_id, $year_id, $day_id, $period_label_id, $grade_id, $class_id, $room_id, $subject_id, $teacher_id, 0)";
                            }
                        ...
                        case 4:
                            ...
                            $sql = "SELECT * FROM schoollms_schema_userdata_school_timetable_items WHERE timetabl_id = $teacher_timetable_id AND year_id = $year_id AND day_id = $day_id AND period_label_id = $period_label_id";
                            $timetable_settings = sqlQuery($sql);

                            if (!empty($timetable_settings)) {
                                $sql = "UPDATE schoollms_schema_userdata_school_timetable_items SET grade_id = $grade_id , class_id = $class_id , room_id = $room_id , subject_id = $subject_id , teacher_id = $teacher_id , substitude_id = 0 WHERE timetabl_id = $teacher_timetable_id AND year_id = $year_id AND day_id = $day_id AND period_label_id = $period_label_id";
                            } else {
                                $sql = "INSERT INTO schoollms_schema_userdata_school_timetable_items (timetabl_id,year_id,day_id, period_label_id,grade_id,class_id, 
										room_id,subject_id,teacher_id,substitude_id) VALUES ($teacher_timetable_id, $year_id, $day_id, $period_label_id, $grade_id, $class_id, $room_id, $subject_id, $teacher_id, 0)";
                            }
...
//                            }
                ...
                $sql = "SELECT * FROM schoollms_schema_userdata_school_timetable_items WHERE timetabl_id = $timetable_id AND year_id = $year_id AND day_id = $day_id AND period_label_id = $period_label_id";
                $timetable_settings = sqlQuery($sql);
                ...
                if (!empty($timetable_settings)) {
                    $sql = "UPDATE schoollms_schema_userdata_school_timetable_items SET grade_id = $grade_id , class_id = $class_id , room_id = $room_id , subject_id = $subject_id , teacher_id = $teacher_id , substitude_id = 0 WHERE timetabl_id = $timetable_id AND year_id = $year_id AND day_id = $day_id AND period_label_id = $period_label_id";
                } else {
                    $sql = "INSERT INTO schoollms_schema_userdata_school_timetable_items (timetabl_id,year_id,day_id, period_label_id,grade_id,class_id, 
                ...
                if (!empty($learner_settings)) {
                    foreach ($learner_settings as $learner_data) {
                        ...
                        if (!empty($timetable_settings)) {
                            ...
                            $sql = "INSERT INTO schoollms_schema_userdata_school_timetable_items (timetabl_id,year_id,day_id, period_label_id,grade_id,class_id, 
								room_id,subject_id,teacher_id,substitude_id) VALUES ($learner_timetable_id,$year_id,$day_id, $period_label_id, $grade_id, $class_id, $room_id, $subject_id, $teacher_id, 0)";
                        }
...
function timetable_store($type, $timetable_id, $class_id, $slot_data){
    ...
    foreach ($slot_data as $day => $period_data) {
        foreach ($period_data as $period => $slot_details) {
            ...
            switch($type){
                case 'update':
                    $q = "UPDATE schoollms_schema_userdata_school_timetable_items SET subject_id = $subject_id WHERE timetabl_id = $timetable_id AND year_id = $year_id AND day_id = $day AND period_id = $period";
                    break;
                ...
                case 'insert':
                    ...
                    $teacher_id = 0;
                    $substitute_id = 0;
                    $q = "INSERT INTO schoollms_schema_userdata_school_timetable_items (timetable_id, year_id, day_id, period_id, room_id, subject_id, teacher_id, substitute_id) VALUES ($timetable_id, $year_id,$day, $period, $room_id, $subject_id, $teacher_id, $substitute_id)";
                    break;
    ...
    switch($save_type){
        ...
        case 'new_user':
            ...
            if (count($save_tokens) > 3){
                ...
                $user_id = 0;
                $user_tokens = explode("*", $save_tokens[1]);
                $user_type = $user_tokens[0];
                $first_name = $user_tokens[1];
                $surname = $user_tokens[2];
                $string = $save_tokens[2];
            }
            
            if ($user_id !== 0){
                $q = "SELECT * FROM schoollms_schema_userdata_access_profile WHERE user_id = '$user_id'";
            } else { 
                $q = "SELECT * FROM schoollms_schema_userdata_access_profile WHERE access_id = '$id' AND name = '$first_name' AND surname = '$surname' AND school_id = $school_id AND type_id = $user_type";
       	   //echo "<br> $q <br>";
            }
...
function timetable_audit_log($user_id, $action, $data){
    
    $today = date('d-m-Y H:i:s');
    
    $q = "INSERT INTO schoollms_schema_securitydata_user_audits VALUES ('$today', '$action', '$user_id', '$data')";
    
    $result = sqlQuery($q);
    
}

function html_get_bgcolor($type, $value){
    ...
    switch($type){
        ...
        case "priority":
            ...
            switch($value){
                ...
                case "Critical":
                //case "High":
                    $bgcolor = "bgcolor='#FA586F'";
                    //$bgcolor = "bgcolor='#FA5858'";
                    break;

                case "Resolved":
                    $bgcolor = "bgcolor='#BEF781'";
                    break;
                ...
                case "Medium":
                    //$bgcolor = "bgcolor='#F5BCA9'";
                    $bgcolor = "bgcolor='#FA8258'";
                    break;

                case "Duplicate":
                    $bgcolor = "bgcolor='#BDBDBD'";
                    break;

                case "Low":
                //case "Enhancement-Deferred":
                    $bgcolor = "bgcolor='#F4FA58'";
                    break;

                case "Tester-Attention":
                    $bgcolor = "bgcolor='#819FF7'";
                    break;
        ...
        case "status":
            ...
            switch($value){
                ...
                case "Enhancement-Declined":
                    $bgcolor = "bgcolor='#9966CC'";
                    break;
                
                case "Enhancement-Confirmed":
                    $bgcolor = "bgcolor='#FFCC99'";
                    break;
                
                case "Enhancement-Disputed":
                    $bgcolor = "bgcolor='#FF9966'";
                    break;
                
                case "User-Response":
                    $bgcolor = "bgcolor='#FF99CC'";
                    break;
            
                case "Recurring-Service":
                    $bgcolor = "bgcolor='#66997A'";
                    break;
                //
                default:
                    $bgcolor = " ";
