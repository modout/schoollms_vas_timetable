...
function get_timetable_slot_links($timetable_id, $date_stamp, $day_id, $period_label_id, $class_id, $user_type, $user_id){
	...
	$link_data = parse_url($actual_link);
	$the_link = "";
	if($link_data["port"] == "" || $link_data["port"] == "80")
	{
		$the_link = $link_data["scheme"]."://".$link_data["host"];
	}
	else{
		$the_link = $link_data["scheme"]."://".$link_data["host"].":".$link_data["port"];
	}
	//echo $the_link;
    $links = "";
    ...
    //if (strcmp($date_stamp,$today_token2) == 0){ 
       $q = "SELECT lesson_url,lesson_title,subject_id FROM schoollms_schema_userdata_school_timetable_subject_lessons WHERE day_id = $day_id AND period_label_id = $period_label_id AND date_format(str_to_date(lesson_date,'%d-%b-%Y'),'%d-%m-%Y') = '$today_token2'";
            ...
            switch ($user_type) {
                case 2:
                   ...
                   // }
					...
					$url = "$the_link/learn/local_timetable_schoollms_link.php";
                ...
                case 4:
					...
					$url = "$the_link/teach/local_timetable_schoollms_link.php";
	...
	for ($col=1; $col <= count($period_labels); $col++) {
		...
		if (array_key_exists($pos, $rs)) {
			...
			for ($i=0; $i < count($elements); $i++) {
				...
				print "<div id=\"$id\" class=\"drag $class\" onClick=\"viewTimeTableSlot('$dname[0]','$datas[0]','CLASS $dclass','$start-$end~$day~$theperiod~$time_table_id')\" >$dname[0]</div>
				$removeSlot
