...
while($row=$data->getRow())
{
	...
	for($i =1;$i<= $periods;$i++)
	{
		...
		if($data1->numrows < 1)
		{
			...
			{
				$sql = "insert into schoollms_schema_userdata_school_timetable_period (week_day_id,school_id,period_start, period_end,period_label_id )
				values('$row->week_day_id','$school_id','$periodstarttime','$periodstarttime','$period_label_id')";
				$data1->execNonSql($sql);
				echo "<br/> $sql <br/>";
			}
			else{
				$row2 = $data1->getRow();
				$sql = "update schoollms_schema_userdata_school_timetable_period set week_day_id = '$row->week_day_id' ,
				school_id = '$school_id',period_start = '$periodstarttime', 
				period_end = '$periodstarttime' ,
				period_label_id = '$period_label_id'
				where period_id = $row2->period_id";
				$data1->execNonSql($sql);
				echo "update some thing here '$periodstarttime','$endtime','$period_label_id' <br/> ";
			}
		}
		...
		{
			$seconds  = strtotime("$day $periodstarttime") - strtotime("$day $break_time_1");
			echo "Seconds ... $periodstarttime  --- $break_time_1 ---  $seconds  <br/>";
			if($seconds >= 0)
			{
				$starttime = "$day $break_time_1";
				$breakstart = "$day $break_time_1";
