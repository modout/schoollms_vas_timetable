...
//include("PhpMailerClass/SchoolLmsMailer.php");
...
$lms_teach_url = "$protocol://$ip/teach/local_timetable_schoollms_link.php";
$lms_learn_url = "$protocol://$ip/learn/local_timetable_schoollms_link.php";
$lms_manage_url = "$protocol://$ip/manage/local_timetable_schoollms_link.php";
$lms_admin_url = "$protocol://$ip/admin/local_timetable_schoollms_link.php";
...
if (strtoupper($action) == 'SENDNOTIFICATIONS'){

    switch($process){
        
        case 'attendance_register':
            $q = "SELECT * FROM schoollms_schema_userdata_learner_attendance_record";
            $data->execSQL($q);
            //(learner_id,register_time,is_present,teacher_id,day_date,week_day_label,period_label,subject,year_id,school_id,class) values 
            //('$learner_id',now(),0,'$teacher_id','$daydata[2]','$daydata[1]','$period_label','$subject','$year_id','$school_id','$class')
            $messages = "";
            while ($row = $data->getRow()){
                $teacher_id = $row->teacher_id;
                $learner_id = $row->learner_id;
                $now = $row->register_time;
                $day_date = $row->day_date;
                $week_day = $row->week_day_label;
                $period = $row->period_label;
                $subject = $row->subject;
                $class = $row->class;
                
                $process = 'attendance_register';
                $process_data = "ABSENT,$learner_id,$now,$day_date,$week_day,$period,$subject,$class";
                $messages .= "<br>".sendNotificationEmail($teacher_id, $process, $process_data, true);
                echo "$messages";
            }
            break;
...
if(strtoupper($action) == 'GETTODAYDAY'){
...
if(strtoupper($action) == 'GETTRAININGLINK'){
    ...
    $lesson_url = "course/21281/schoollms-training";//quiz/22479/open-chatroom";
    $lesson_title = "SchoolLMS Training";
    //&date=$date_stamp&day=$day_id&period_label_id=$period_label_id&user_type=$user_type
    $pars = "action=open_link&q=$lesson_url&username=$username&passwd=$passwd&user_id=$user_id&lesson_title=$lesson_title";
    
    echo "$url?$pars";
}

if(strtoupper($action) == 'GETTEACHERCLASSES'){
    
    $classes = array ();
    $q = "SELECT * FROM schoollms_schema_userdata_school_classes WHERE class_id IN (SELECT class_id FROM schoollms_schema_userdata_teacher_schooldetails WHERE school_id = $school_id AND year_id = $year_id AND user_id = $user_id)";
    $data->execSQL($q);
...
if(strtoupper($action) == 'STATSREPORT'){
//    echo "I GET HERE $type<br>";
    $report = array ();
    switch($type){
        case 'class_attendance':
            //GET SYS ARCH
            $out = array();
            shell_exec("wmic cpu get DataWidth", $out);
...
//             }
            ...
            //Get Year Start Date
            $q = "SELECT start_time FROM schoollms_schema_userdata_school_year_calendar WHERE calendar_type = 1 AND year_id = $year_id";
//            echo "SQL $q <br>";
            $data->execSQL($q);
            //$data = new Data("db");
//            $data->prepareDataDatabaseQuery($q);
...
//            $start = $data->db_query->fetchColumn();//$row['start_time'];//"2017|01|02|00|00|00"
            ...
            $year_start_date_items = explode("|", $start_time);
//            echo "START TIME $start_time <br>";
        ...
        //    $count_grade = $from_grade;
           // while ($count_grade <= $to_grade){
                //GET NUM LEARNERS
                $q = "SELECT DISTINCT * FROM schoollms_schema_userdata_learner_schooldetails WHERE class_id IN (SELECT class_id FROM schoollms_schema_userdata_teacher_schooldetails WHERE school_id = $school_id AND year_id = $year_id AND user_id = $user_id AND class_id = $class_id)";
//                echo "Q $q <br>";
//                $data->prepareDataDatabaseQuery($q);
...
//                $data->db_query->execute($q_data);
                $data->execSQL($q);
                $total_class_learners = $data->numrows;

                $row = $data->getRow();
                $class_label = $row->class_label;
                $class_label_tokens = explode(" ", $class_label);
                if (count($class_label_tokens) == 2){
                    $class_color_label = $class_label;
                } else {
                    $count_labels = count($class_label_tokens);
                    $class_color_label = $class_label_tokens[$count_labels-2]." ".$class_label_tokens[$count_labels-1];
                }
                ...
                $num_days = dateDifference($start_date_token, $today_token, '%a');//timetable_date_difference($message);
                ...
                $total_class_learners_present = 0;
                $class_attendance = array ();
                while ($count_days <= $num_days){
                    //$message = "$last_day!~!1!~!0!~!0";
                    $next_day =  add_date($last_day,1,0,0);//timetable_add_date($message);
                    $next_day_tokens = explode(" ", $next_day);
                    $total_class_learners_absent = 0;
                    ...
                    if (in_array($day_items[0], $weekends)){
                        //Do Nothing
                    } elseif (isHoliday($next_day)){//timetable_day_is_holiday($next_day)) {//Check Holidays
                        //Do Nothing
                    } else {
                        ...
                        $time = timetable_adjust_time(64, $time);

                        $q = "SELECT DISTINCT(learner_id) FROM schoollms_schema_userdata_learner_attendance_record WHERE day_date = '$day_string2' AND class = '$class_label'";
//                        $data->prepareDataDatabaseQuery($q);
//                        $q_data = array ($day_string2,"CLASS%$count_grade%");
//                        $data->db_query->execute($q_data);
                        $data->execSQL($q);
                        $total_class_learners_absent = $data->numrows;
                        //$total_grade_learners_absent = count($data->db_query->fetchAll());
                        $total_class_learners_present = $total_class_learners - $total_class_learners_absent;

                        $class_attendance[$time] = $total_class_learners_present;
...
//                        echo "BITS $bits DAY $day_string STRTIME $time TOTAL GRADE $count_grade LEARNERS $total_grade_learners ABSENT $total_grade_learners_absent PRESENT $total_grade_learners_present<br>";
                ...
                $stats = array ();
                foreach ($class_attendance as $time => $value) {
        
       
                    $item = array ($time, $value);
                    array_push($stats, $item);
                }
                //exit;
                $color = html_get_bgcolor('chart-classes', "$class_color_label");
                
                $report = array ('stats'=>$stats, 'color'=>$color);
...
//            $q = "SELECT settings FROM schoollms_schema_userdata_timetable_settings WHERE school_id = $school_id";
////            echo "SQL $q <br>";
//            $data->execSQL($q);
...
//            echo "START TIME $start_time <br>";
        ...
        //    $count_grade = $from_grade;
           // while ($count_grade <= $to_grade){
                //GET NUM LEARNERS
                $q = "SELECT DISTINCT * FROM schoollms_schema_userdata_learner_schooldetails WHERE grade_id = $grade_id AND class_id IN (SELECT class_id FROM schoollms_schema_userdata_school_classes WHERE year_id = $year_id AND class_label LIKE 'CLASS%')";
//                echo "Q $q <br>";
//                $data->prepareDataDatabaseQuery($q);
//
//                $q_data = array ($count_grade,$year_id,"CLASS%");
...
//                $data->db_query->execute($q_data);
                $data->execSQL($q);
                $total_grade_learners = $data->numrows;
                ...
                //Weekends
                $weekends = array ("Sat", "Sun");
                $total_grade_learners_present = 0;
                $grade_attendance = array ();
                while ($count_days <= $num_days){
                    //$message = "$last_day!~!1!~!0!~!0";
                    $next_day =  add_date($last_day,1,0,0);//timetable_add_date($message);
                    $next_day_tokens = explode(" ", $next_day);
                    $total_grade_learners_absent = 0;
                    //echo "<br> NEXT DAY $next_day";
                    ...
                    if (in_array($day_items[0], $weekends)){
                        ...
                        $q = "SELECT DISTINCT(learner_id) FROM schoollms_schema_userdata_learner_attendance_record WHERE day_date = '$day_string2' AND class LIKE 'CLASS%$count_grade%'";
//                        $data->prepareDataDatabaseQuery($q);
//                        $q_data = array ($day_string2,"CLASS%$count_grade%");
//                        $data->db_query->execute($q_data);
                        $data->execSQL($q);
                        $total_grade_learners_absent = $data->numrows;
                        //$total_grade_learners_absent = count($data->db_query->fetchAll());
                        $total_grade_learners_present = $total_grade_learners - $total_grade_learners_absent;

                        $grade_attendance[$time] = $total_grade_learners_present;
...
//                        echo "BITS $bits DAY $day_string STRTIME $time TOTAL GRADE $count_grade LEARNERS $total_grade_learners ABSENT $total_grade_learners_absent PRESENT $total_grade_learners_present<br>";
                ...
                $stats = array ();
                foreach ($grade_attendance as $time => $value) {
                ...
                //exit;
                $color = html_get_bgcolor('chart-grade', "grade$grade_id");
...
if(strtoupper($action) == 'GETLESSONS'){
...
//    echo "SUBJECTID Q $q <br>\n";
    ...
    if ($data->numrows == 1){
        ...
        $result[] = "$subject is NOT FOUND for query <br> $q <br>- Fix Database";
//            exit;
    } else {
        $result[] = "There are more than one $subject - Clean Database";
        //exit;
    }
...
//        echo "<hr/> CLASSID $q <hr/>";
        ...
        if ($data->numrows == 1){
            ...
            $result[] = "$class is NOT FOUND for query <br> $q <br>- Fix Database";
//            exit;
        } else {
            $result[] = "There are more than one $class for query <br> $q <br>- Clean Database";
//            exit;
        }
...
//            echo "WEEKID Q $q <br>\n";
            ...
            if ($data->numrows == 1){
                ...
                $result[] = "$week_day is NOT FOUND for query <br> $q <br>- Fix Database";
    //            exit;
            } else {
                $result[] = "There are more than one $week_day for query <br> $q <br>- Clean Database";
    //            exit;
            }
...
//                $result[] = $q;
                ...
                if ($data->numrows == 1){
                    ...
                    $result[] = "period_label_id is NOT FOUND for query <br> $q <br>- Fix Database";
        //            exit;
                } else {
                    $result[] = "There are more than one period_label_id for query <br> $q <br>- Clean Database";
        //            exit;
                }
                ...
                if ($result[0]){
                    ...
                    if ($data->numrows == 1){
                        ...
                        $result[] = "$day is NOT FOUND for query <br> $q <br>- Fix Database";
            //            exit;
                    } else {
                        $result[] = "There are more than one day_id for query <br> $q <br>- Clean Database";
            //            exit;
                    }
...
//                        $result[] = $sql;
                        ...
                        $q = "SELECT * FROM schoollms_schema_userdata_school_timetable_subject_lessons WHERE day_id = $day_id AND period_label_id = $period_label_id AND class_id = $class_id AND subject_id = $subject_id AND lesson_date = '$date[0]' AND lesson_url = '$lessonurl'";
//                         $result[] = $q;
                        $data->execSQL($q);
                        if ($data->numrows == 0){
                            $q = "INSERT INTO schoollms_schema_userdata_school_timetable_subject_lessons (day_id,period_label_id,class_id,subject_id,lesson_date,lesson_url,lesson_title) VALUES ($day_id, $period_label_id, $class_id, $subject_id, '$date[0]', '$lessonurl', '$lesson')";
//                            $result[] = $q;
                            $data->execNonSql($q);
...
if(strtoupper($action) == 'GETSCHOOLYEARS'){
    
    //YEAR
    $q = "SELECT * FROM schoollms_schema_userdata_school_year WHERE year_id IN (select distinct(year_id) from schoollms_schema_userdata_teacher_timetable where school_id = $school_id)";
...
if(strtoupper($action) == 'GETSCHOOLSUBJECTS'){
    ...
    //SUBJECT
    if ($year_id == 0 && $grade_id == 0){
        $q = "SELECT * FROM schoollms_schema_userdata_school_subjects WHERE subject_id IN (select distinct(subject_id) from schoollms_schema_userdata_teacher_schooldetails where school_id = $school_id)";
    } elseif ($year_id !== 0 && $grade_id == 0){
        $q = "SELECT * FROM schoollms_schema_userdata_school_subjects WHERE subject_id IN (select distinct(subject_id) from schoollms_schema_userdata_teacher_schooldetails where school_id = $school_id AND year_id = $year_id)";
    } elseif ($year_id !== 0 && $grade_id !== 0){
        $q = "SELECT * FROM schoollms_schema_userdata_school_subjects WHERE subject_id IN (select distinct(subject_id) from schoollms_schema_userdata_teacher_schooldetails where school_id = $school_id AND year_id = $year_id AND grade_id = $grade_id)";
    } elseif ($year_id == 0 && $grade_id !== 0){
        $q = "SELECT * FROM schoollms_schema_userdata_school_subjects WHERE subject_id IN (select distinct(subject_id) from schoollms_schema_userdata_teacher_schooldetails where school_id = $school_id AND grade_id = $grade_id)";    
    } else {
        $q = "SELECT * FROM schoollms_schema_userdata_school_subjects WHERE subject_id IN (select distinct(subject_id) from schoollms_schema_userdata_teacher_schooldetails where school_id = $school_id AND year_id = $year_id AND grade_id = $grade_id)";
    }
...
if(strtoupper($action) == 'GETSCHOOLTEACHERS'){
    ...
    if ($grade_id == 0 && $subject_id == 0){
        $q = "SELECT * FROM schoollms_schema_userdata_access_profile WHERE type_id = 4 AND user_id IN (select distinct(user_id) from schoollms_schema_userdata_teacher_schooldetails where school_id = $school_id AND year_id = $year_id)";
    } elseif ($grade_id !== 0 && $subject_id == 0) {
        $q = "SELECT * FROM schoollms_schema_userdata_access_profile WHERE type_id = 4 AND user_id IN (select distinct(user_id) from schoollms_schema_userdata_teacher_schooldetails where school_id = $school_id AND year_id = $year_id AND grade_id = $grade_id)";
    } elseif ($grade_id == 0 && $subject_id !== 0) {
        $q = "SELECT * FROM schoollms_schema_userdata_access_profile WHERE type_id = 4 AND user_id IN (select distinct(user_id) from schoollms_schema_userdata_teacher_schooldetails where school_id = $school_id AND year_id = $year_id AND subject_id = $subject_id)";
    } else {
        $q = "SELECT * FROM schoollms_schema_userdata_access_profile WHERE type_id = 4 AND user_id IN (select distinct(user_id) from schoollms_schema_userdata_teacher_schooldetails where school_id = $school_id AND year_id = $year_id AND grade_id = $grade_id AND subject_id = $subject_id)";
    }
...
if(strtoupper($action) == 'GETSCHOOLCLASSES'){
    ...
    //CLASS
    if ($grade_id == 0 && $subject_id == 0 && $teacher_id == 0){
        $q = "SELECT * FROM schoollms_schema_userdata_school_classes WHERE class_id IN (select distinct(class_id) from schoollms_schema_userdata_teacher_schooldetails where school_id = $school_id AND year_id = $year_id)";
    } elseif ($grade_id !== 0 && $subject_id == 0 && $teacher_id == 0) {
        $q = "SELECT * FROM schoollms_schema_userdata_school_classes WHERE class_id IN (select distinct(class_id) from schoollms_schema_userdata_teacher_schooldetails where school_id = $school_id AND year_id = $year_id AND grade_id = $grade_id)";
    } elseif ($grade_id !== 0 && $subject_id !== 0 && $teacher_id == 0) {
        $q = "SELECT * FROM schoollms_schema_userdata_school_classes WHERE class_id IN (select distinct(class_id) from schoollms_schema_userdata_teacher_schooldetails where school_id = $school_id AND year_id = $year_id AND grade_id = $grade_id AND subject_id = $subject_id)";
    } elseif ($grade_id == 0 && $subject_id !== 0 && $teacher_id !== 0) {
        $q = "SELECT * FROM schoollms_schema_userdata_school_classes WHERE class_id IN (select distinct(class_id) from schoollms_schema_userdata_teacher_schooldetails where school_id = $school_id AND year_id = $year_id AND subject_id = $subject_id)";    
    } elseif ($grade_id !== 0 && $subject_id == 0 && $teacher_id !== 0) {
        $q = "SELECT * FROM schoollms_schema_userdata_school_classes WHERE class_id IN (select distinct(class_id) from schoollms_schema_userdata_teacher_schooldetails where school_id = $school_id AND year_id = $year_id AND grade_id = $grade_id AND teacher_id = $teacher_id)";    
    } else {
        $q = "SELECT * FROM schoollms_schema_userdata_school_classes WHERE class_id IN (select distinct(class_id) from schoollms_schema_userdata_teacher_schooldetails where school_id = $school_id AND year_id = $year_id AND grade_id = $grade_id AND subject_id = $subject_id AND teacher_id = $teacher_id)";
    }
...
if(strtoupper($action) == 'GETSCHOOLLEARNERS'){
    ...
    //LEARNERS
    if ($grade_id == 0 && $subject_id == 0 && $teacher_id == 0 && $class_id == 0){
        $q = "SELECT * FROM schoollms_schema_userdata_access_profile WHERE type_id = 2 AND user_id IN (select distinct(user_id) from schoollms_schema_userdata_learner_schooldetails where school_id = $school_id AND year_id = $year_id)";
    } elseif ($grade_id !== 0 && $subject_id == 0 && $teacher_id == 0 && $class_id == 0) {
        $q = "SELECT * FROM schoollms_schema_userdata_access_profile WHERE type_id = 2 AND user_id IN (select distinct(user_id) from schoollms_schema_userdata_learner_schooldetails where school_id = $school_id AND year_id = $year_id AND grade_id = $grade_id)";
    } elseif ($grade_id !== 0 && $subject_id !== 0 && $teacher_id == 0 && $class_id == 0) {
        $q = "SELECT DISTINCT ap.* FROM schoollms_schema_userdata_access_profile ap "
                . "JOIN schoollms_schema_userdata_learner_schooldetails ls ON ls.user_id = ap.user_id"
                . "JOIN schoollms_schema_userdata_learner_timetable lt ON lt.user_id = ls.user_id"
                . "JOIN schoollms_schema_userdata_school_timetable_items ti ON ti.timetabl_id = lt.timetabl_id"
                . "WHERE ap.type_id = 2 AND ls.school_id = $school_id AND ls.year_id = $year_id AND ls.grade_id = $grade_id AND ti.subject_id = $subject_id";
    } elseif ($grade_id !== 0 && $subject_id !== 0 && $teacher_id !== 0 && $class_id == 0) {
        $q = "SELECT DISTINCT ap.* FROM schoollms_schema_userdata_access_profile ap "
                . "JOIN schoollms_schema_userdata_learner_schooldetails ls ON ls.user_id = ap.user_id"
                . "JOIN schoollms_schema_userdata_learner_timetable lt ON lt.user_id = ls.user_id"
                . "JOIN schoollms_schema_userdata_school_timetable_items ti ON ti.timetabl_id = lt.timetabl_id"
                . "WHERE ap.type_id = 2 AND ti.teacher_id = $teacher_id AND ls.school_id = $school_id AND ls.year_id = $year_id AND ls.grade_id = $grade_id AND ti.subject_id = $subject_id";
   } elseif ($grade_id == 0 && $subject_id !== 0 && $teacher_id !== 0 && $class_id !== 0) {
        $q = "SELECT DISTINCT ap.* FROM schoollms_schema_userdata_access_profile ap "
                . "JOIN schoollms_schema_userdata_learner_schooldetails ls ON ls.user_id = ap.user_id"
                . "JOIN schoollms_schema_userdata_learner_timetable lt ON lt.user_id = ls.user_id"
                . "JOIN schoollms_schema_userdata_school_timetable_items ti ON ti.timetabl_id = lt.timetabl_id"
                . "WHERE ap.type_id = 2 AND ti.teacher_id = $teacher_id AND ls.school_id = $school_id AND ls.year_id = $year_id AND ti.subject_id = $subject_id AND ti.class_id = $class_id";
    } elseif ($grade_id !== 0 && $subject_id == 0 && $teacher_id !== 0 && $class_id !== 0) {
        $q = "SELECT DISTINCT ap.* FROM schoollms_schema_userdata_access_profile ap "
                . "JOIN schoollms_schema_userdata_learner_schooldetails ls ON ls.user_id = ap.user_id"
                . "JOIN schoollms_schema_userdata_learner_timetable lt ON lt.user_id = ls.user_id"
                . "JOIN schoollms_schema_userdata_school_timetable_items ti ON ti.timetabl_id = lt.timetabl_id"
                . "WHERE ap.type_id = 2 AND ti.teacher_id = $teacher_id AND ls.school_id = $school_id AND ls.year_id = $year_id AND ls.grade_id = $grade_id";    
    } elseif ($grade_id !== 0 && $subject_id !== 0 && $teacher_id == 0 && $class_id !== 0) {
        $q = "SELECT DISTINCT ap.* FROM schoollms_schema_userdata_access_profile ap "
                . "JOIN schoollms_schema_userdata_learner_schooldetails ls ON ls.user_id = ap.user_id"
                . "JOIN schoollms_schema_userdata_learner_timetable lt ON lt.user_id = ls.user_id"
                . "JOIN schoollms_schema_userdata_school_timetable_items ti ON ti.timetabl_id = lt.timetabl_id"
                . "WHERE ap.type_id = 2 AND ls.school_id = $school_id AND ls.year_id = $year_id AND ls.grade_id = $grade_id";
    } elseif ($grade_id == 0 && $subject_id == 0 && $teacher_id !== 0 && $class_id !== 0) {
        $q = "SELECT DISTINCT ap.* FROM schoollms_schema_userdata_access_profile ap "
                . "JOIN schoollms_schema_userdata_learner_schooldetails ls ON ls.user_id = ap.user_id"
                . "JOIN schoollms_schema_userdata_learner_timetable lt ON lt.user_id = ls.user_id"
                . "JOIN schoollms_schema_userdata_school_timetable_items ti ON ti.timetabl_id = lt.timetabl_id"
                . "WHERE ap.type_id = 2 AND ti.teacher_id = $teacher_id AND ls.school_id = $school_id AND ls.year_id = $year_id AND ti.class_id = $class_id";    
    } elseif ($grade_id == 0 && $subject_id == 0 && $teacher_id == 0 && $class_id !== 0) {
        $q = "SELECT DISTINCT ap.* FROM schoollms_schema_userdata_access_profile ap "
                . "JOIN schoollms_schema_userdata_learner_schooldetails ls ON ls.user_id = ap.user_id"
                . "JOIN schoollms_schema_userdata_learner_timetable lt ON lt.user_id = ls.user_id"
                . "JOIN schoollms_schema_userdata_school_timetable_items ti ON ti.timetabl_id = lt.timetabl_id"
                . "WHERE ap.type_id = 2 AND ls.school_id = $school_id AND ls.year_id = $year_id AND ti.class_id = $class_id";   
    } elseif ($grade_id == 0 && $subject_id == 0 && $teacher_id !== 0 && $class_id == 0) {
        $q = "SELECT DISTINCT ap.* FROM schoollms_schema_userdata_access_profile ap "
                . "JOIN schoollms_schema_userdata_learner_schooldetails ls ON ls.user_id = ap.user_id"
                . "JOIN schoollms_schema_userdata_learner_timetable lt ON lt.user_id = ls.user_id"
                . "JOIN schoollms_schema_userdata_school_timetable_items ti ON ti.timetabl_id = lt.timetabl_id"
                . "WHERE ap.type_id = 2 AND ti.teacher_id = $teacher_id AND ls.school_id = $school_id AND ls.year_id = $year_id";
    } elseif ($grade_id == 0 && $subject_id !== 0 && $teacher_id !== 0 && $class_id == 0) {
        $q = "SELECT DISTINCT ap.* FROM schoollms_schema_userdata_access_profile ap "
                . "JOIN schoollms_schema_userdata_learner_schooldetails ls ON ls.user_id = ap.user_id"
                . "JOIN schoollms_schema_userdata_learner_timetable lt ON lt.user_id = ls.user_id"
                . "JOIN schoollms_schema_userdata_school_timetable_items ti ON ti.timetabl_id = lt.timetabl_id"
                . "WHERE ap.type_id = 2 AND ti.teacher_id = $teacher_id AND ls.school_id = $school_id AND ls.year_id = $year_id AND ti.subject_id = $subject_id";
    } elseif ($grade_id == 0 && $subject_id !== 0 && $teacher_id == 0 && $class_id == 0) {
        $q = "SELECT DISTINCT ap.* FROM schoollms_schema_userdata_access_profile ap "
                . "JOIN schoollms_schema_userdata_learner_schooldetails ls ON ls.user_id = ap.user_id"
                . "JOIN schoollms_schema_userdata_learner_timetable lt ON lt.user_id = ls.user_id"
                . "JOIN schoollms_schema_userdata_school_timetable_items ti ON ti.timetabl_id = lt.timetabl_id"
                . "WHERE ap.type_id = 2 AND ls.school_id = $school_id AND ls.year_id = $year_id AND ti.subject_id = $subject_id";    
    } else {
        $q = "SELECT DISTINCT ap.* FROM schoollms_schema_userdata_access_profile ap "
                . "JOIN schoollms_schema_userdata_learner_schooldetails ls ON ls.user_id = ap.user_id"
                . "JOIN schoollms_schema_userdata_learner_timetable lt ON lt.user_id = ls.user_id"
                . "JOIN schoollms_schema_userdata_school_timetable_items ti ON ti.timetabl_id = lt.timetabl_id"
                . "WHERE ap.type_id = 2 AND ti.class_id = $class_id AND ti.teacher_id = $teacher_id AND ls.school_id = $school_id AND ls.year_id = $year_id AND ls.grade_id = $grade_id AND ti.subject_id = $subject_id";
    }
...
//    if ($grade_id == 0 && $subject_id == 0 && $teacher_id == 0 && $class_id == 0 && $learner_id == 0){
//        $q = "SELECT * FROM schoollms_schema_userdata_access_profile WHERE type_id = 3 AND user_id IN (select distinct(user_id) from schoollms_schema_userdata_learner_schooldetails where school_id = $school_id AND year_id = $year_id)";
//    } elseif ($grade_id !== 0 && $subject_id == 0 && $teacher_id == 0 && $class_id == 0 && $learner_id == 0) {
//        $q = "SELECT * FROM schoollms_schema_userdata_access_profile WHERE type_id = 2 AND user_id IN (select distinct(user_id) from schoollms_schema_userdata_learner_schooldetails where school_id = $school_id AND year_id = $year_id AND grade_id = $grade_id)";
//    } elseif ($grade_id !== 0 && $subject_id !== 0 && $teacher_id == 0 && $class_id == 0 && $learner_id == 0) {
//        $q = "SELECT DISTINCT(ap.*) FROM schoollms_schema_userdata_access_profile ap "
//                . "JOIN schoollms_schema_userdata_learner_schooldetails ls ON ls.user_id = ap.user_id"
...
//    //GRADE
//    if ($year_id == 0){
//        $q = "SELECT * FROM schoollms_schema_userdata_school_grades WHERE grade_id IN (select distinct(grade_id) from schoollms_schema_userdata_learner_schooldetails where school_id = $school_id)";
//    } else {
        $q = "SELECT * FROM schoollms_schema_userdata_school_grades WHERE grade_id IN (select distinct(grade_id) from schoollms_schema_userdata_learner_schooldetails where school_id = $school_id AND year_id = $year_id)";
//    }    
    $data->execSQL($q);
...
if(strtoupper($action) == "GETVENUES")
{
  //  echo "I GET HERE<br>";
    ...
    $sql = "select * from schoollms_schema_userdata_school_building_rooms where room_type = $type and school_id = $school_id";
//    echo "Q $sql <br>";
//    exit;
...
//        $class_label = $row->class_label;
//        $class = array ();
//        if (strpos($class_label, "/") !== FALSE){
//            $class_tokens = explode("/", $class_label);
//            
//            foreach ($close_tokens as $key => $label) {
//                $class[] = $label;
//            }
//        } else {
//            $class[] = $class_label;
//        }
        ...
        //IF TRUE SKIP
        //ELSE ADD TO LMS
//        foreach ($class as $key => $label) {
//            $q = "SELECT * FROM node WHERE title = '$label' AND type = 'class'";
//            $data2->execSQL($q);
//            if ($data2->numrows == 0){
//                //$url = "http://127.0.0.1:8081/teach/local_timetable_schoollms_link.php";
//                $pars = "action=save_new_class&username=System Admin&passwd=$0W3t0&class_label=$label&grade_no=-1";
//
...
if(strtoupper($action) == "MODIFYCLASSTEACHER")
{
    ...
    //GET subject_id 
    if (!empty($class_id) && !empty($grade_id)){
        ...
        if ($data->numrows > 0){
            ...
            if (!empty($subject_id) && !empty($class_id) && !empty($grade_id)){
...
if(strtoupper($action) == "GETTEACHERGRADES")
{
        $sql = "select * from schoollms_schema_userdata_school_grades where grade_id in (select distinct(grade_id) from schoollms_schema_userdata_teacher_details where user_id = $user_id and school_id = $school_id and year_id = $year_id)";
        $data->execSQL($sql);
        $result = array();
        while($row = $data->getRow())
        {
...
?>
	...
	<?php
//     //CHECK IF USER LMS STATUS IS ACTIVE FOR ITEM IN THEIR PROFILE (I.E SUBJECTS AND CLASSES)
//     //IF ACTIVE - MEANS THE USER HAS LMS ACCOUNT AND THEIR TIMETABLE PROFILE IS LINKED TO THE LMS
//     //ELSE CHECK IF ACCOUNT EXISTS
//     //IF EXISTS - LINK USER TIMETABLE PROFILE TO LMS
//     //IF DOES NOT EXIST - CREATE USER LMS ACCOUNT AND THEN LINK USER TIMETABLE PROFILE
//        
//     $timetable_db = "sphslms_qa_vas_timetable";    
//     switch ($user_type) {
//         case 2:
//            $lms_db = "sphslms_qa_learn";
//            $pars = "action=add_user_profile&lms_db=$lms_db&timetable_db=$timetable_db&username=System Admin&passwd=$0W3t0&user_id=$user_id&year_id=$year_id";
////
//            $contents = do_post_request($lms_learn_url,$pars);
//             break;
//         
//         case 4:
//            
//            $lms_db = "sphslms_qa_teach";
//            $pars = "action=add_user_profile&lms_db=$lms_db&timetable_db=$timetable_db&username=System Admin&passwd=$0W3t0&user_id=$user_id&year_id=$year_id";
////
//            $contents = do_post_request($lms_teach_url,$pars);
////            //echo "RESPONSE $contents PARA $pars<br>\n";
//            //$remote_response = json_decode($contents, TRUE);
//
//             break;
...
if(strtoupper($action) == "TIMETABLESELECT" )
{
       // $q = "SELECT * FROM schoollms_schema_userdata_school_classes  sc join SELECT * FROM schoollms_schema_userdata_school_timetable st WHERE sc.school_id = $school_id AND sc.grade_id = '$timetable_label' AND timetable_type_id IN (3,8) AND sc.class_id = timetable_type_item_id order by class_label";
        $q =  "SELECT distinct st.timetabl_id, st.timetable_label FROM schoollms_schema_userdata_school_classes sc join schoollms_schema_userdata_school_timetable st on sc.school_id = 1 AND sc.year_id = $year_id AND sc.grade_id = '$timetable_label' AND sc.class_id = timetable_type_item_id AND st.timetable_type_id IN (3,8) order by st.timetable_label";
...
if(strtoupper($action) == "GETIMAGE")
{
	...
	$sql = "select * from schoollms_schema_userdata_user_photo where user_id = ?";
...
//        echo "TIMESLOT $timeslot <br>";
        //2017-01-14 14:36:00-~<b>Day 8</b><br>Sun<br>15-Jan-2017~4221~4~1743 <br>
	...
	//var_dump($daydata);
	//var_dump($timedata);
	$sql = "select * from schoollms_schema_userdata_school_timetable where timetabl_id = '$timedata[4]'";
	$data->execSQL($sql);
	if($row = $data->getRow())
	{
        ...
        //GET PERIOD LABEL
        $q = "SELECT * FROM schoollms_schema_userdata_school_timetable_period_labels WHERE period_label_id = $timedata[3]";
        $data->execSQL($q);
        $period_label = "";
	if($row = $data->getRow())
	{
	...
	$nowparts = explode(" ",$now);
        $nowparts = explode(":",$nowparts[1]);
	$startparts = explode(":",$start_time);
	$endparts = explode(":",$end_time);
	
	//echo "START $start_time END $end_time NOW $now CHECK $startparts[0] <= $nowparts[0] and $endparts[0] >= $nowparts[0]";
        //exit;
	if((int) $startparts[0] <= (int) $nowparts[0] && (int) $endparts[0] >= (int) $nowparts[0])
	{
		$sql = "";
		if($ispresent=="true")
		{
			...
			week_day_label = '$daydata[1]' and period_label= '$period_label' and 
			...
			'$daydata[1]','$period_label','$subject','$year_id','$school_id','$class')";
		}
//		echo "<br/>$sql";
		...
		$data->execNonSql($sql);
                ...
                $process = 'attendance_register';
                $process_data = "$result,$learner_id,$now,$daydata[2],$daydata[1],$period_label,$subject,$class";
                sendNotificationEmail($teacher_id, $process, $process_data);
	} elseif ((int) $endparts[0] <= (int) $nowparts[0]){
               $result = "PAST";                         
        } elseif ((int) $startparts[0] >= (int) $nowparts[0]){
               $result = "COMING";                                
        } else{
		$result = "WRONGTIMESLOT";
	}
...
function sendNotificationEmail($user_id, $process, $process_data, $return_message=null){
    ...
    $name = $row->name;
    $surname = $row->surname;
    $teacher_fullname = "$name $surname";
    
    //Get USER EMAIL
    //$q = "SELECT * FROM schoollms_schema_userdata_contact_details WHERE user_id = $user_id";
    //$data->execSQL($q);
    //$row = $data->getRow();
    $email = "modise@ekasiit.com";//$row->email;
    
    switch($process){
        
        case 'attendance_register':
            //"$learner_id,$now,$day_date,$week_day,$period,$subject,$class";
            $process_data_tokens = explode(",", $process_data);
            $attendance = $process_data_tokens[0];
            $learner_id = $process_data_tokens[1];
            ...
            $name = $row->name;
            $surname = $row->surname;
            $learner_fullname = "$surname, $name";
            
            $message  = "Good Day $teacher_fullname<br/><br/>The following learner attendance register was processed.<br/><br/>";
            $message_table  = "<table><tr><th>Learner</th><th>Attendance</th><th>Register Time</th><th>Day Date</th><th>Week Day</th><th>Period</th><th>Subject</th><th>Class</th><th>Teacher</th></tr>"
                            . "<tr><td>$learner_fullname</td><td>$attendance</td><td>$now</td><td>$day_date</td><td>$week_day</td><td>$period</td><td>$subject</td><td>$class</td><td>$teacher_fullname</td></tr></table><br/>";
            $message  .= "$message_table<br/>";
            break;
        
        case 'manage_attendance_register':
            
            break;
            
        case 'parent_attendance_register':
    ...
    SchoolLmsMailer::ProcessMail($email,$fullname,$message,"$process: Notification");
